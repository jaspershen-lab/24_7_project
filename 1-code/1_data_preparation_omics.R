...
setwd("3_data_analysis/combine_omics/data_preparation")
...
write.csv(new_expression_data, "new_expression_data.csv", row.names = FALSE)
write.csv(new_sample_info, "new_sample_info.csv", row.names = FALSE)
write.csv(new_variable_info, "new_variable_info.csv", row.names = FALSE)
...
plot =
  ...
  dplyr::arrange(data_type, median) %>%
  ggplot() +
  gghalves::geom_half_violin(aes(x = data_type,
                                 y = median,
                                 color = data_type),
                             show.legend = FALSE) +
  ...
  labs(x = "", y = "Spearman correlation range")

plot
ggsave(plot,
       filename = "real_loess_comparision.pdf",
       height = 4,
       width = 8)
